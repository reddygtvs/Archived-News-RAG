...
│ │ ├── (Generated files like guardian_articles.jsonl, article_lookup.pkl,
│ │ │ news_index.faiss, metadata.json, evaluation_results_v2.jsonl,
│ │ │ plots_final_automated/ - these are gitignored but generated by scripts)
│ ├── .env.example # Example environment file (DO NOT COMMIT .env)
│ ├── config.py # Configuration settings
│ ├── requirements.txt # Python dependencies
│ ├── data_fetcher.py # Script to fetch data from Guardian API
│ ├── build_index.py # Script to process data and build FAISS index/lookups
│ ├── rag_core.py # Core RAG logic, LLM calls, LLM evaluation
│ ├── evaluate.py # Script to run automated evaluation
│ ├── plot_results.py # Script to generate plots from evaluation results
│ ├── app.py # Flask API server for the web interface
│ ├── test_queries.json # Predefined queries for evaluation <--- CORRECTED LOCATION
